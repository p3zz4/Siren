...
export class Siren {
	constructor() {
		...
		this.RxJSShowHide(this.canvas, this.leftPanel, this.rightPanel, this.bottomPanel);
	}
	initAudio() {
	...
	initLeftPanel(layerParamsArray) {
		...
		function updateLayerContainer(layerContainer, layerParamsArray) {
			...
			layerParamsArray.forEach((data, index) => {
				...
				layer.addEventListener("drop", event => {
					event.preventDefault();
					const draggedLayerIndex = event.dataTransfer.getData("draggedLayerIndex");
					swapLayers(layerParamsArray, index, draggedLayerIndex);
					updateLayerContainer(layerContainer, layerParamsArray);
				})
				const moveBtn = domElementCreator("div", layer, "layerBtn");
				moveBtn.draggable = true;
				moveBtn.innerHTML = `<i class="fas fa-expand-arrows-alt"></i>`;
				moveBtn.addEventListener("dragstart", event => {
					event.dataTransfer.setData("draggedLayerIndex", index);
				});
				const deleteBtn = domElementCreator("div", layer, "layerBtn");
		...
		function swapLayers(layerParamsArray, firstElementIndex, secondElementIndex) {
			const tmp = layerParamsArray[firstElementIndex];
			layerParamsArray[firstElementIndex] = layerParamsArray[secondElementIndex];
			layerParamsArray[secondElementIndex] = tmp;
		}
	...
	initRightPanel(audio, dbLocation) {
		...
		// audio.addEventListener("play", _=>{
		// 	const index = audio.sourceElements.indexOf(audio.firstChild);
		// 	console.log(playlist, playlist[index])
		// 	playlist[index].style.border = "1px solid red";
		// })
		return rightPanel;
	}
	...
	RxJSShortcuts(audio, playPauseBtn) {
	...
	RxJSShowHide(cancelElement) {
		const args = [...arguments].slice(1);
		args.forEach(element => {
			RxJS.Observable.fromEvent(element, "mouseover").subscribe( _ => {
				args.forEach(element => {
					element.classList.remove("hover");
				});
				element.classList.add("hover");
			})
			RxJS.Observable.fromEvent(cancelElement, "click").subscribe( _ => {
				element.classList.remove("hover");
			})
		})
	}
